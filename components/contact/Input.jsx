    ...
    <div className={cn("p-2 w-1/2", className)}>
      ...
      {multiline ? (
        <textarea
          ...
          type={type}
          className={cn(
            "w-full bg-gray-300 rounded border border-gray-300 focus:border-textGreen text-[#0a192f] focus:ring-2 focus:ring-textGreen/50 h-14 text-base outline-none py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out"
          )}
...
const InputError = ({ message }) => {
  return (
    <motion.p
      className="flex items-center gap-1 h-4 font-bold text-red-500 text-[11px] font-secondary"
      {...framer_error}
    >
      <MdError size={14} />
      {message}
